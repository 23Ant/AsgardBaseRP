...
Citizen.CreateThread(function()
	while true do
		Citizen.Wait(1)
			...
			local d2 = GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()),304.0085144043,-600.55609130859,43.283992767334,true)
			local d3 = GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()),-196.36163330078,-1319.2924804688,31.089347839355,true)
			local drota = GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()),-2049.2658691406,-462.7639465332,12.240750312805,true)
			local carta = GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()),238.91275024414,-1381.5561523438,33.741779327393,true)
			local porte = GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()),447.35006713867,-975.84020996094,30.689661026001,true)
			local hsp = GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()),340.01400756836,-585.96514892578,28.791481018066,true)
			
			local slot = GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()),-57.222038269043,-1097.3762207031,26.417385101318,true)
			...
			if hsp <= 3.0  then
				...
				if IsControlJustPressed(0,38) then
					...
					PlaySoundFrontend(-1,"Enter_1st","GTAO_FM_Events_Soundset",false)
					SetEntityCoords(PlayerPedId(),346.73303222656,-576.04406738281,29.500198364258)
			...
			if d2 <= 3.0  then
			    DrawText3D(304.0085144043,-600.55609130859,43.283992767334, "PRESSIONE  ~b~E~w~  PARA BATER O CARTAO ")
				if IsControlJustPressed(0,38) then
			...
			if d3 <= 3.0  then
			    DrawText3D(-196.36163330078,-1319.2924804688,31.089347839355, "PRESSIONE  ~b~E~w~  PARA BATER O CARTAO ")
				if IsControlJustPressed(0,38) then
			...
			if drota <= 3.0  then
			    DrawText3D(-2049.2658691406,-462.7639465332,12.240750312805, "PRESSIONE  ~b~E~w~  PARA BATER O CARTAO ")
				if IsControlJustPressed(0,38) then
			...
			if carta <= 3.0  then
			    DrawText3D(238.91275024414,-1381.5561523438,33.741779327393, "PRESSIONE  ~b~E~w~  PARA COMPRAR CARTA ")
				if IsControlJustPressed(0,38) then
			...
			if porte <= 3.0  then
			    DrawText3D(447.35006713867,-975.84020996094,30.689661026001, "PRESSIONE  ~b~E~w~  PARA COMPRAR PORTE DE ARMAS ")
				if IsControlJustPressed(0,38) then
			...
			if slot <= 3.0  then
			    DrawText3D(-57.222038269043,-1097.3762207031,26.417385101318, "PRESSIONE  ~b~E~w~  PARA COMPRAR SLOT ")
				if IsControlJustPressed(0,38) then
				    TriggerServerEvent("vrp_slot")
				end
			end
			if slot2 <= 3.0  then
