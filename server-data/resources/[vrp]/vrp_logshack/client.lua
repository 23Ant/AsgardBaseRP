...
AddEventHandler('explosionEvent', function(sender, ev)
	...
	if (idExplosao == 29) then
		PerformHttpRequest("https://discordapp.com/api/webhooks/673519957228912680/qpfHGa4v8wdq0uT6bRm_Y9IAIidZ8PP3pfc9rJ1h9eEXhC9ONvovAD5WxCFvysw2rARl", function(err, text, headers) end, 'POST', json.encode({content =  "**Explosões** ```" .. "ID: " .. user_id .. " Nome: " .. jogador .. "\nServidor: " .. GetConvar('sv_name', 'ND') .. "\nData: " .. os.date("%H:%M:%S %d/%m/%Y") .. "```"}), { ['Content-Type'] = 'application/json' })
	end
