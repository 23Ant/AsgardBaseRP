...
local webuse = "https://discordapp.com/api/webhooks/731947263710069038/74s15AuokBIq10QiMGGYfYefuDqEtPfZvC9QUoVvLUWLTnn5LkOixdYe-uSW3sN3dD66"
local webdropar = "https://discordapp.com/api/webhooks/731947263710069038/74s15AuokBIq10QiMGGYfYefuDqEtPfZvC9QUoVvLUWLTnn5LkOixdYe-uSW3sN3dD66"
local webgive = "https://discordapp.com/api/webhooks/731947263710069038/74s15AuokBIq10QiMGGYfYefuDqEtPfZvC9QUoVvLUWLTnn5LkOixdYe-uSW3sN3dD66"
...
AddEventHandler("vrp_inventario:dropItem",function(data)
        ...
        if vRPclient.isInComa(player) then
            ...
            if idname == "mochila" then		
				...
				local msg = "``` [ID: "..source.."] dropou uma mochila```"
                SendWebhookMessage(webdropar,msg)
            end
            if peso then
                if idname == "vale" then		
                ...
                elseif vRP.tryGetInventoryItem(user_id, data.idname, amount, false) then
					...
					local msg = "``` [ID: "..source.."] dropou "..amount.."x de "..idname.."```"
                    SendWebhookMessage(webdropar,msg)
    ...
    if vRPclient.isInComa(player) then
        ...
        if type == "weapon" or type == "ammo" or type == "mochila" or type == "crack" or type == "lsd"or type == "maconha" or type == "energetico" or type == "cerveja" or type == "absinto" or type == "tequila" or type == "vodka" or type == "conhaque" or type == "whisky" or type == "bandagem" or type == "cocaina"or type == "metanfetamina" or type == "lockpick" or type == "capuz" or type == "masterpick" then
            ...
            if vRP.tryGetInventoryItem(user_id, idname, tonumber(amount), false) then
				...
				local msg = "``` [ID:"..user_id.."] usou "..amount.."x de "..idname.."```"
                SendWebhookMessage(webuse,msg)
            end
        end
        if type == "ammo" then
            local fullidname = split(idname, "|")
            ...
            if type == "mochila" then
                local src = source
					if vRP.tryGetInventoryItem(user_id,"mochila",1) then
						...
						local msg = "``` [ID:"..user_id.."] usou 1x Mochila```"
                        SendWebhookMessage(webuse,msg)
					else
            ...
            if type == "capuz" then
                if vRP.getInventoryItemAmount(user_id,"capuz") >= 1 then
                    ...
                    if nplayer then
						...
						local msg = "``` [ID:"..user_id.."] usou "..amount.."x de "..idname.."em [ID: "..nplayer.."```"
                        SendWebhookMessage(webuse,msg)
                    else
        ...
        if type == "colete" then  
		    if vRP.tryGetInventoryItem(user_id,"colete1",1) then
			    local msg = "``` [ID:"..user_id.."] usou "..amount.."x de Colete Intacto```"
                SendWebhookMessage(webuse,msg)
		        local colete = 100
		...
		if type == "coleteme" then  
		    if vRP.tryGetInventoryItem(user_id,"colete2",1) then
			    local msg = "``` [ID:"..user_id.."] usou "..amount.."x de Colete Danificado```"
                SendWebhookMessage(webuse,msg)
		        local colete = 50
		...
		if type == "coleteus" then  
            if vRP.tryGetInventoryItem(user_id,"colete3",1) then
                local msg = "``` [ID:"..user_id.."] usou "..amount.."x de Colete Perfurado```"
                SendWebhookMessage(webuse,msg)			
		        local colete = 20
        ...
        if type == "none" then
            TriggerClientEvent("chatMessage",player,"ALERTA",{255,70,50},"Este item não pode ser usado.")            
        end
    end    
end
    ...
    if user_id then
        ...
        if nplayer then
            ...
            if nuser_id then
                ...
                if amount > 0 then
				    if data.idname == "vale" then
					    TriggerClientEvent("chatMessage",source,"ALERTA",{255,70,50},"Não pode ser enviado .")
                    end 
                    if vRP.getInventoryWeight(nuser_id)+vRP.getItemWeight(data.idname)*amount <= vRP.getInventoryMaxWeight(nuser_id) then
                        if vRP.tryGetInventoryItem(user_id,data.idname,amount,true) then
                            ...
                            TriggerClientEvent("chatMessage",source,"ALERTA",{255,70,50},"Enviou ^1"..data.idname.." x"..amount.."^0.")
                            TriggerClientEvent("chatMessage",nplayer,"ALERTA",{255,70,50},"Recebeu ^1"..data.idname.." x"..amount.."^0.")
							
							local msg = "``` [ID: "..source.."] enviou para [ID:"..nplayer.."] x"..amount.." de "..data.idname.."```"
                            SendWebhookMessage(webgive,msg)
							
                        end
                    else
                        TriggerClientEvent("chatMessage",source,"ALERTA",{255,70,50},"Inventário cheio.")
                        TriggerClientEvent("chatMessage",nplayer,"ALERTA",{255,70,50},"Inventário cheio.")
                    end
                end
            end
        end
