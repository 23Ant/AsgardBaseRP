...
function func.checkRobbery(id,x,y,z,head)
	...
	deucerto = parseInt(math.random(1,5))

	if vRP.tryGetInventoryItem(user_id,"macarico",1,true) then
	if deucerto == 1 or deucerto == 2 or deucerto == 4 or deucerto == 5 then
	    if user_id then 
		    if #policia < 2 then
		    	...
		    	for l,w in pairs(policia) do
		            ...
		            if player then
					    async(function()
						    ...
						    TriggerClientEvent('chatMessage',player,"COPOM",{245,0,0},"O roubo começou no ^1Caixa Eletrônico^0, dirija-se até o local e intercepte os assaltantes.")
					    end)
				    end
			    end
	...
	elseif deucerto == 3 then 
	    TriggerClientEvent("Notify",source,"aviso","Falhou corre!!!!!!!!!.")
		for l,w in pairs(policia) do
		local player = vRP.getUserSource(parseInt(w))
		    if player then
			    async(function()
	    ...
	    TriggerClientEvent("Notify",source,"aviso","Sem maçarico.")
	end
end
-----------------------------------------------------------------------------------------------------------------------------------------
-- CANCELROBBERY
