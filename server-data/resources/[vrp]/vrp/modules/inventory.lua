...
local webhooklink1 = "https://discordapp.com/api/webhooks/721890146148024333/l_k03H35bIdDGigu6mEK6iw7ho6nG1Xb6RKqGhVSmv1noO10iWvvXRmwuooRQBAoyiVl"
vRP.items = {}
...
function vRP.clearInventory(user_id)
	...
	if data then
	    ...
	    local aaaaaa = data.inventory["vale"]
		local quant = aaaaaa.amount
		
		local dia = data.inventory["dias"]
		local diaquant = dia.amount
		
		local horas = data.inventory["horas"]
		local horasquant = horas.amount
		
		local minutos = data.inventory["minutos"]
		local minutosquant = minutos.amount
		
		
		data.inventory = {}
		data.inventory["dias"] = { amount = diaquant }
		data.inventory["horas"] = { amount = horasquant }
		data.inventory["minutos"] = { amount = horasquant }
		data.inventory["vale"] = { amount = quant }
	end
end
function vRP.clearInventory2(user_id)
	local data = vRP.getUserDataTable(user_id)
	if data then
		data.inventory = {}
	end
...
local function build_itemlist_menu(name,items,cb)
	...
	for k,v in pairs(items) do 
		...
		if name then
		    if name == "vale" or name == "Vale" or name == "dias" or name == "horas" or name == "minutos" then
			else
			    kitems[name] = k
		...
		if data.inventory then
			if not chests[name] then
				...
				local cb_put = function(idname)
					if string.match(idname,"dinheirosujo") or string.match(idname,"vale") or string.match(idname,"dias") or string.match(idname,"horas") or string.match(idname,"minutos") then
		...
		if data.inventory then
			if not chests[name] then
				...
				local cb_put = function(idname)
					...
					amount = parseInt(amount)
					local new_weight = vRP.computeItemsWeight(chest.items)+vRP.getItemWeight(idname)*amount
					if string.match(idname,"vale") or string.match(idname,"dias") or string.match(idname,"horas") or string.match(idname,"minutos") then
						TriggerClientEvent("Notify",source,"importante","Não pode guardar <b>Dinheiro Sujo</b> em veículos.")
						return
					end--aqui
					if new_weight <= max_weight then
