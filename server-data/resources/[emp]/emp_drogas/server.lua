...
function emP.checkPayment()
	...
	if user_id then
		if vRP.tryGetInventoryItem(user_id,"maconha",quantidade[source]) or vRP.tryGetInventoryItem(user_id,"cocaina",quantidade[source]) or vRP.tryGetInventoryItem(user_id,"metanfetamina",quantidade[source]) then
			vRP.giveInventoryItem(user_id,"dinheirosujo",math.random(40,50)*quantidade[source])
		...
		for l,w in pairs(soldado) do
			...
			if player then
				async(function()
					...
					TriggerClientEvent('chatMessage',player,"COPOM",{245,0,0},"Recebemos uma denuncia de drogas, verifique o ocorrido.")
