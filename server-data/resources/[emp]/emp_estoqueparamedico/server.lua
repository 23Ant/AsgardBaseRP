...
AddEventHandler('DonSpeed:DescarregandoPara', function()
	...
	GuardarValor[1] = parseInt(items["morfina"].amount)
	items["morfina"].amount = nil
	items["morfina"] = nil 

    GuardarValor[2] = parseInt(items["desfibrilador"].amount)
	items["desfibrilador"].amount = nil
	items["desfibrilador"] = nil 
	
	GuardarValor[3] = parseInt(items["bandagem"].amount)
	items["bandagem"].amount = nil
	items["bandagem"] = nil 
	
	GuardarValor[4] = parseInt(items["pills"].amount)
    ...
    if dsitems["morfina"] == nil then
	    dsitems["morfina"] = { amount = parseInt(GuardarValor[1]) } 
    else
	    dsitems["morfina"].amount = dsitems["morfina"].amount + GuardarValor[1]
    end
	if dsitems["desfibrilador"] == nil then
	    dsitems["desfibrilador"] = { amount = parseInt(GuardarValor[2]) } 
    else
	    dsitems["desfibrilador"].amount = dsitems["desfibrilador"].amount + GuardarValor[1]
    end
	if dsitems["bandagem"] == nil then
	    dsitems["bandagem"] = { amount = parseInt(GuardarValor[1]) } 
    else
	    dsitems["bandagem"].amount = dsitems["bandagem"].amount + GuardarValor[1]
    end
	if dsitems["pills"] == nil then
	    dsitems["pills"] = { amount = parseInt(GuardarValor[1]) } 
    else
	    dsitems["pills"].amount = dsitems["pills"].amount + GuardarValor[1]
    end
